# Claude Code Token Lens

![Claude Code Token Lens](public/screenshot-placeholder.png) <!-- Replace this with an actual screenshot later -->

**Claude Code Token Lens** is a beautiful, local-first usage tracker and dashboard for AI coding agents. 

Initially built to parse local logs for **Claude Code**, the architecture is designed to be platform-agnostic, allowing future support for other AI agents (Cursor, Aider, GitHub Copilot CLI, etc.). It aggregates token usage, calculates exact costs, and visualizes daily trends with rich, interactive drill-down capabilities (Project > Session > Conversation > Turns).

## ✨ Features

- 🔒 **Local-First & Secure**: Runs entirely on your local machine. It reads your agent's local JSON logs without sending your private codebase or prompt history to any external server.
- 📊 **Detailed Dashboards**: View your total cost, token consumption, and active days at a glance.
- 📈 **Daily Trends & Aggregation**: Beautiful bar charts and line graphs showing input/output/cache tokens and costs per day.
- 🔍 **Deep Drill-Down UI**: Seamlessly filter and navigate from **Projects** -> **Sessions** -> **Conversations** -> **Individual Turns**.
- 🏷️ **Smart Filtering**: Inline filter bubbles make it easy to see exactly what context you are viewing, with one-click clearing.
- 💵 **Cost Tracking**: Uses the latest pricing models (e.g., Claude 3.5 Sonnet) to accurately compute your API costs based on real usage.

## 🚀 Getting Started

### Prerequisites

- Node.js 18+ 
- Local AI Agent logs (currently supports `~/.claude/projects/*.jsonl`)

### Installation

1. Clone the repository:
   ```bash
   git clone https://github.com/yourusername/ai-token-lens.git
   cd ai-token-lens
   ```

2. Install dependencies:
   ```bash
   npm install
   ```

3. Run the development server:
   ```bash
   npm run dev
   ```

4. Open [http://localhost:3000](http://localhost:3000) with your browser to see your usage dashboard!

## 🛣️ Roadmap
- [x] Claude Code (`.claude/projects/`) log parsing support
- [ ] Add support for Cursor AI logs
- [ ] Add support for Aider logs
- [ ] Custom pricing configuration interface
- [ ] Date-range filtering (e.g., Last 7 Days, This Month)

## 🙏 Acknowledgements / Inspiration

This project was inspired by the need to track API costs generated by autonomous coding agents. The initial parser logic was inspired by community efforts to demystify local `.jsonl` logs generated by tools like Claude Code.

## 📄 License

This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.
